...
def count_words_in_file(filename):
    """
    Reads a file and counts the number of occurrences of words in it.
    Whitespace is ignored.

    Parameters:
    - filename, string : path to the file to count
...
def count_words_in_batch(filenames):
    counts = dict()
    for filename in filenames:
        merge_counts(counts, count_words_in_file(filename))
    return counts
...
if __name__ == '__main__':
    ...
    print(f'Time for listing files: {time_measurement_a_end - time_measurement_a_start:.2f} seconds')

    time_measurement_b_start = time.time()
    batches = [filenames[i:i + batch_size] for i in range(0, len(filenames), batch_size)]
    with mp.Pool(num_workers) as pool:
        file_counts = pool.map(count_words_in_batch, batches)
    time_measurement_b_end = time.time()
    print(f'Time for reading and counting words in files: {time_measurement_b_end - time_measurement_b_start:.2f} seconds')
