...
JOB_SCRIPT="${SCRIPT_DIR}/assignment2_problem2e_job.sh"
COLLECT_SCRIPT="${SCRIPT_DIR}/assignment2_problem2e_collect_results.sh"
...
for workers in "${WORKERS[@]}"; do
  ...
  job_ids+=("$job_id")
  echo "submitted problem2e workers=${workers}, cpus=${workers}, job_id=${job_id}"
done

echo
echo "After all problem2e jobs finish, run this on the login node:"
