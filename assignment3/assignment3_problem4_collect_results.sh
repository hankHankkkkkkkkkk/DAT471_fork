...
SUMMARY_FILE="${RESULT_DIR}/problem4_summary.txt"
...
for workers in "${WORKERS[@]}"; do
...

  {
    ...
    if [[ -f "$result_file" ]]; then
      grep -E '^(workers=|cpus_per_task=|dataset=|most followers id|most followers|average followers|count no followers|Number of workers:|Time elapsed:)' "$result_file" || true
